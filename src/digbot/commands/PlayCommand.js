...
module.exports = class PlayCommand extends Command {
    ...
    async execute({ message: msg }) { // eslint-disable-line consistent-return
        ...
        if (msg.content.length === 5 || msg.content.substring(6) === ' ') {
            sendMessageToChannel(msg.channel,
                'Please provide a valid command. E.g. !playCommand **video** dQw4w9WgXcQ');
            return false;
        }
        ...
        if (msg.content.substring(6).startsWith('video')) {
            ...
            sendMessageToChannel(msg.channel, 'Sorry I didn\'t recognise that command, please '
                + 'use "!help playCommand" for a more detailed explanation of the command if you\'re confused');
            return false;
        }
    }
    ...
    static passList() {
        ...
        let message = '__**Play Command:**__'
            ...
            + '\n**Commands**:'
            + '\nvideo: Used to playCommand the audio from a video in your channel, remember to '
        ...
        message += '\n'
            + '\n**Examples:**'
            + '\n*!playCommand video dQw4w9WgXcQ*'
            + '\n*!playCommand playlist nightcore*'
            + '\n*!playCommand playlist PLAEQD0ULngi67rwmhrkNjMZKvyCReqDV4*'
            + '\n*!playCommand stop*'
            + '\n*!playCommand skip*'
            + '\n*!playCommand volume down*';
...
function playCommand() {
    ...
    if (server.getReady() === false) {
        logger.debug(TAG, 'Server not ready, setting playCommand on timeout');
        setTimeout(() => {
            logger.event(TAG, 'server not ready setTimeout in playCommand()');
            logger.debug(TAG, 'Calling playCommand again');
            playCommand();
        }, 10000);
        return false;
    }
        ...
        playing[x].bot.channels.get(x)
            ...
            .then((connection) => {
                ...
                connection.on('error', (err) => {
                    kill(x);
                    setTimeout(() => {
                        logger.event(TAG, 'error setTimeout in playCommand()');
                        playCommand();
                    }, 15000);
            ...
            .catch((err) => {
                ...
                kill(x);
                setTimeout(() => {
                    logger.event(TAG, 'catch setTimeout in playCommand()');
                    playCommand();
                }, 15000);
            });
    }
